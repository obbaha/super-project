...
<x-layouts.app title="تفاصيل القطعة">
    ...
    <div x-data="{
        ...
        {{-- تحويل التنوعات إلى مصفوفة JS --}}
        variations: @js($this->product->variations->map(fn($v) => [
            'id' => $v->id,
            'name' => $v->attribute_name,
            'additional_price' => $v->additional_price,
            'image' => $v->featuredImage ? Storage::url($v->featuredImage->path) : null,
        ...
        {{-- منطق جلب الصورة: صورة التنوع المختار أو أول صورة متاحة للمنتج --}}
        get currentImage() {
            if (this.selectedVariation && this.selectedVariation.image) {
                return this.selectedVariation.image;
            }
            return '{{ $this->product->variations->whereNotNull('featuredImage')->first()?->featuredImage ? Storage::url($this->product->variations->whereNotNull('featuredImage')->first()->featuredImage->path) : '' }}';
        },
        get isAvailable() {
...
<div class="space-y-6" data-aos="fade-right">
    ...
    {{-- الألبوم المصغر (كما في الكود القديم) --}}
        ...
        <template x-for="variation in variations" :key="variation.id">
            <template x-if="variation.image">
                ...
                >
                    ...
                    {{-- علامة بصرية إذا كان الموديل غير متوفر في الألبوم --}}
                    <template x-if="!variation.isAvailable">
                        <div class="absolute inset-0 bg-black/20 flex items-center justify-center">
                            <x-icon name="o-no-symbol" class="w-4 h-4 text-white/50" />
                        </div>
                    </template>
                </button>
            </template>
